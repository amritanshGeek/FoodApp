    ...
    ({ scrollY }) => {
      ...
      return (
        <Animated.View style={[styles.header, headerStyle, headerAnimatedStyles]}>
          ...
          <TouchableOpacity style={{padding:2}} onPress={()=>dispatch(emptyOrderHistoryData())}>
          ...
          <View bg={'muted.50'} shadow={5} flex={1} width={Dimensions.get('window').width-50} flexDirection={'row'} justifyContent={'space-around'} alignItems={'center'} py={10} >
            ...
            <HStack>
              <Button bg={'muted.50'} onPress={()=>{
                ...
                <Icon as={Entypo} name='info-with-circle' color={'orange.400'} size={10} />
              </Button>
              <Button bg={'muted.50'} onPress={()=>dispatch(removefromOrderHistory(item))}>
                <Icon as={Entypo} name='squared-cross' color={'orange.400'} size={10} />
              </Button>
            </HStack>
          </View>
        )}
